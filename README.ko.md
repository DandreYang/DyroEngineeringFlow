...
`--discover`는 로컬 Git 저장소를 검색해 워크스페이스 상대 경로와 개발 라인 내 위치를 자동 등록하고, 가능하면 `origin`도 읽습니다. TOML을 직접 편집할 필요가 없습니다. 아직 저장소가 없다면 대화형 대안을 사용하세요.
...
| 명령 | 용도 |
| --- | --- |
| `init --discover` / `init --wizard` / `repo add/list` / `bootstrap` / `start` | TOML 편집 없는 온보딩, anchor 관리, 개발 라인과 Agent 선택. |
...
| `agent list` / `open` | adapter 확인 또는 올바른 개발 라인에서 Agent 실행. |
...
| `task claim` / `task evidence execution/review` | 격리 runner의 일회성 작업 수령과 receipt, 게이트, 작업 HEAD, 검토 증거 가져오기. |
...
DyroEngineeringFlow는 로컬 워크플로를 완결하고 엄격한 팀을 로컬 계획 전용 모드로 유지하는 정책 제어를 제공합니다. remote 저장소 생성, SaaS 자격 증명, 외부 runner 구현은 포함하지 않습니다. 외부 runner는 Profile 확장으로 연결합니다. [MIT License](LICENSE)로 제공되며 [PyPI의 `dyro`](https://pypi.org/project/dyro/)로 배포되었습니다.
