"""Stage 5 Supervisor: host provider, dual cleanup, pack, evidence dry-run.

Closes the local experiment path. Never imports into Dyro Core, never
signoff/merge/push. Production remains Not ready (see production_gate).
"""
...
from ..stage4.evidence_pack import CleanupProof, EvidencePackResult, pack_run_evidence
from ..stage4.worktree_quota import WorktreeQuota
from ..supervisor import Stage0Supervisor, SupervisorConfig, SupervisedResult
from .docker_broker import Stage5DockerBrokerStack
from .evidence_dry_run import (
    EvidenceDryRunResult,
    dry_run_validate_pack,
    refuse_production_actions,
)
from .host_provider import HostProviderPin, assert_host_pin_in_identity
from .production_gate import assert_not_production_ready, evaluate_production_readiness
...
PROVIDER_TOKEN = "stage5-broker-only-token"


@dataclass(frozen=True)
class Stage5SupervisorConfig:
    bundle_root: Path
...
class Stage5Supervisor:
    def __init__(self, config: Stage5SupervisorConfig) -> None:
        self.config = config
        self._bundle_manifest = deepcopy(dict(config.bundle_manifest))
        self._bundle_identity = deepcopy(dict(config.bundle_identity))
        refuse_production_actions(config.production_actions)
        self._assert_identity()
        self._host_provider = config.host_provider
        assert_host_pin_in_identity(self._bundle_identity, self._host_provider)
        self._host_provider.verify()
    ...
    def _assert_identity(self) -> None:
        ...
        if (
            ...
            or self._bundle_identity.get("stage") != 5
            or self._bundle_identity.get("production_ready") is not False
            or evidence.get("requires_dual_cleanup") is not True
            or evidence.get("dry_run_validator") is not True
            or evidence.get("allows_core_import") is not False
            ...
            raise Stage0ValidationError(
                "Stage 5 bundle identity is not the approved closeout configuration"
            )

    def execute(self) -> Stage5RunResult:
        refuse_production_actions(self.config.production_actions)
        ...
        if EXECUTION_KEY_ENV in os.environ or EXECUTION_KEY_SENTINEL in os.environ:
            raise Stage0ValidationError(
                "execution key must not be present before Stage 5 isolation starts"
            )
        ...
        # Host provider path and token must not appear on sandbox-visible run root.
        host_path_str = str(self._host_provider.host_path)
        for path in self.config.run_root.rglob("*"):
            if not path.is_file():
                continue
            text = path.read_text(encoding="utf-8", errors="ignore")
            if PROVIDER_TOKEN in text or host_path_str in text:
                raise Stage0ValidationError(
                    "provider secret or host path leaked onto sandbox-visible run root"
                )
        ...
        try:
            ...
            sandbox = DockerSandboxConfig(
                ...
                environment={
                    ...
                    "DYRO_STAGE5_PHASE1_MS": str(matrix.phase1_hold_ms),
                    "DYRO_STAGE5_PHASE3_MS": str(matrix.phase3_hold_ms),
                    "HOME": "/tmp",
                    "TMPDIR": "/tmp",
        ...
        host_path_leaked = host_path_str in report_text or host_path_str in telemetry_text
        if leaked or token_leaked or host_path_leaked:
            raise Stage0ValidationError(
                "provider raw material, token, or host path leaked into surfaces"
            )

        usage = self._quota.assert_within(self.config.worktrees)

        os.environ[EXECUTION_KEY_ENV] = "stage5-simulated-key-not-for-production"
        try:
            ...
            dry_run = dry_run_validate_pack(
                pack.pack_dir,
                production_actions=self.config.production_actions,
            )
        finally:
            os.environ.pop(EXECUTION_KEY_ENV, None)

        production_gate = evaluate_production_readiness()
        assert_not_production_ready(production_gate)
        ...
        return Stage5RunResult(
            ...
            provider_mode=self.config.provider_mode,
            host_provider=self._host_provider.to_mapping(),
            raw_marker_leaked=False,
