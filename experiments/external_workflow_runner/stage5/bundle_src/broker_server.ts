/**
 * Stage 5 Agent Broker — host-mounted integrity-pinned argv CLI + raw on tmpfs.
 *
 * Modes:
 * - fake / simulated-cli (compat)
 * - argv-cli: spawn host-mounted pinned provider; capture stdout/stderr to tmpfs
 *
 * Provider credentials exist only in this process. Host provider path is never
 * shared with the Workflow Sandbox. Execution keys are refused.
 */
import {
  appendFileSync,
  existsSync,
...
const providerPinPath =
  process.env.DYRO_PROVIDER_PIN_PATH ?? "/opt/host-provider/provider_cli";
