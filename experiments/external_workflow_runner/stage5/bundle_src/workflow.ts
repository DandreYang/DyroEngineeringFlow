/**
 * Stage 5 multi-phase workflow (host provider path):
 *   phase1 hold -> parallel agent branches via Broker -> phase3 hold
 * Claim renewals, evidence packing, and dry-run are handled by the Supervisor.
 */
...
async function main(): Promise<void> {
  ...
  const phase1 = Number(process.env.DYRO_STAGE5_PHASE1_MS ?? "1200");
  const phase3 = Number(process.env.DYRO_STAGE5_PHASE3_MS ?? "1200");

  // Phase 1: hold (claim renewal window).
  await Bun.sleep(phase1);

  // Phase 2: parallel agent branches through Broker host-mounted provider.
  const results = await parallelSettled(
    input.branches.map(
      (branchId, index) => async () => {
        const summary = await callBroker(
          `Stage5 phase2 analyze ${branchId}`,
          input.model,
          `call-${index + 1}`,
        );
