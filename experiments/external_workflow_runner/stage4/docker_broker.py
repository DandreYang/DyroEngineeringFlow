"""Stage 4 Docker broker: pinned argv-cli provider + dual-cleanup stop proof."""
...
from .provider_pin import ProviderBinaryPin
...
class Stage4DockerBrokerStack:
    ...
    provider_mode: str
    max_concurrency: int
    provider_pin: ProviderBinaryPin
    ...
    def start(
        ...
        *,
        ...
        port: int = 7421,
        provider_fake_token: str = "stage4-broker-only-token",
        ...
        provider_pin.verify(bundle_root)

        token = secrets.token_hex(4)
        network_name = f"dyro-s4-net-{token}"
        netns_name = f"dyro-s4-ns-{token}"
        broker_name = f"dyro-s4-broker-{token}"
        ...
        start_broker = _run_docker(
            [
                ...
                "--env",
                f"DYRO_PROVIDER_ARGV={provider_pin.argv_csv()}",
                "--env",
                f"DYRO_PROVIDER_ARGV_SHA256={provider_pin.content_sha256}",
                "--env",
                f"DYRO_PROVIDER_PIN_PATH=/opt/workflow/{provider_pin.relative_path}",
                "--env",
    ...
    def _wait_until_ready(self, *, timeout_seconds: float = 20.0) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            if "broker-pin-mismatch" in combined:
                raise Stage0ValidationError("broker rejected provider integrity pin")
    ...
    def stop(self) -> None:
        ...
        # Dual cleanup proof: both containers must be gone.
