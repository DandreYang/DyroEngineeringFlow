/**
 * Stage 4 multi-phase workflow:
 *   phase1 hold -> parallel agent branches via Broker -> phase3 hold
 * Claim renewals + evidence packing are handled by the Supervisor.
 */
...
async function main(): Promise<void> {
  ...
  const phase1 = Number(process.env.DYRO_STAGE4_PHASE1_MS ?? "1200");
  const phase3 = Number(process.env.DYRO_STAGE4_PHASE3_MS ?? "1200");
    ...
    input.branches.map(
      (branchId, index) => async () => {
        const summary = await callBroker(
          `Stage4 phase2 analyze ${branchId}`,
          input.model,
          `call-${index + 1}`,
        );
