...
from .adapters.registry import get_adapter, probe_backends
...
def resolve_panel_members(requested: Sequence[str] | None) -> list[str]:
    if requested:
        ...
        for name in requested:
            ...
            if adapter.available() or adapter.id == "echo":
    ...
    available = [row["id"] for row in probe_backends() if row["available"]]
...
def run_panel(
    ...
    *,
    ...
    """
    Fan out the same task to multiple backends (sequential execution under
    slot limits). Writes a panel board JSON under dispatch home.
    """
    base = dict(payload)
